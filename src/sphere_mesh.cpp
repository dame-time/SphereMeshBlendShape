...
#ifdef DEBUG
std::vector<int> BumperGrid::sqrtHistogram;
std::vector<int> BumperGrid::planeHistogram;
#endif
...
#ifdef DEBUG
void savePrysmoidAsSM(const std::string& filename, const Sphere &sa, const Sphere &sb, const Sphere &sc)
{
...
bool BumperGrid::pushOutsideGrid (glm::vec3 &p, glm::vec3 &n) const
{
...
#ifdef BOOK_KEEP
	sqrtHistogram.push_back(0);
	planeHistogram.push_back(0);
#endif
	...
	if (bumper[bumperAtPos].shapeType == Bumper::SPHERE)
	{
		...
		planeHistogram.back() += 1;
#endif
		if (pushOutsideSphere(p, n, bumperAtPos))
		{
#ifdef DEBUG
			sqrtHistogram.back() += 1;
#endif
